...
from pathlib import Path   # noqa: F401 – used elsewhere in the repo
...
# ---------------------------------------------------------------------------#
# 1.  Load the two ONNX graphs once per container                             #
# ---------------------------------------------------------------------------#
MODELS = {
...
for key, path in MODELS.items():
    ...
    # yolov8 ONNX exports are square; take the width as stride
...
def fetch_image(url: str, timeout: int = 10) -> Image.Image:
    """Download image and convert to RGB PIL."""
...
def run_inference(img: Image.Image, model_key: str) -> np.ndarray:
    ...
    # ── FIX ── reshape confidence (N,) → (N, 1) then concat with coords
    coords = logits[:4, keep].T            # (N, 4)
    scores = conf[keep][:, None]           # (N, 1)
    boxes = np.hstack((coords, scores))    # (N, 5)
...
def draw_boxes(img: Image.Image, boxes: np.ndarray) -> Image.Image:
    """Draw red rectangles for each detection."""
...
def pack_jpeg_base64(pil_img: Image.Image) -> dict:
    """Return API‑Gateway‑compatible JSON with base64‑encoded JPEG body."""
    buf = io.BytesIO()
...
# ---------------------------------------------------------------------------#
s3 = boto3.client("s3")
BUCKET = os.environ.get("GEO_BUCKET")  # set by CDK in your stack
...
def handler(event, _context):
    ...
    except Exception as exc:  # surface every error for easy debugging
