...
def _to_heat(mask: np.ndarray) -> np.ndarray:
...
def heatmap_overlay(img: Image.Image, masks: Iterable[np.ndarray] | None = None) -> Image.Image:
    ...
    *If* the caller passes masks they are used; otherwise we run YOLO and take its masks.
    """
    ...
    if masks is None:
        ...
        pred = mdl.predict(source=img, imgsz=640, conf=0.25, retina_masks=True, verbose=False)[0]
